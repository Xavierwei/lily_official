			...
			<tr>
				...
				<td width="10">
					<?php 
						...
						if($key>0&&($item->lft!=$item->parent_node->lft+1)){echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/up.gif">',array('/admin/type/moveUp/','id'=>$item->id));}
					?>
				</td>
				<td width="10">
					<?php
						$item->parent_node->rgt=isset($item->parent_node->rgt)?$item->parent_node->rgt:0;
						if($item->id!=$item->last_brother->id&&($item->rgt!=$item->parent_node->rgt-1)){echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/down.gif">',array('/admin/type/moveDown/','id'=>$item->id));}
					?>
				</td>
